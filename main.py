...
class CPU(object):
    ...
    def op_ld_imm_8(self, dst):
        dst.write(self.bus.read(cpu.PC.read()+1))
        self.PC.incr(2)
        return 8

    def op_ld_imm_16(self, dst):
        dst.write(self.bus.read_16(cpu.PC.read()+1))
        self.PC.incr(3)
        return 12
...
if __name__=="__main__":
    ...
    with open(sys.argv[1], "rb") as f:
        ...
        rom = ROM(f)
        bus.attach(rom, 0x0000, 0x3FFF)

        ram = [RAM(4096), RAM(4096)]
        bus.attach(ram[0], 0xC000, 0xCFFF)
        bus.attach(ram[1], 0xD000, 0xDFFF)
